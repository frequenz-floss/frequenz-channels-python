...
class Selected(Generic[ReceiverMessageT_co]):
    ...
    [`Selected`][frequenz.channels.Selected] instances should be used in conjunction with the
    ...
    def message(self) -> ReceiverMessageT_co:  # noqa: DOC503
        ...
        Raises:
            Exception: If there was an exception while receiving the message. Normally
                this should be an [`Error`][frequenz.channels.Error]
...
def selected_from(
    ...
    """Return whether the given receiver was selected by [`select()`][frequenz.channels.select].
