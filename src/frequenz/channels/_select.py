...
When a single [`Receiver`][] is stopped, it will be reported
via the [`Selected`][] object. You can use the
[`was_stopped`][Selected.was_stopped] method to check if the
selected [`Receiver`][] was stopped:
...
The [`Selected`][] object also has a
...
class Selected(Generic[ReceiverMessageT_co]):
    ...
    [`selected_from()`][] function to determine
...
def selected_from(
    ...
    """Return whether the given receiver was selected by [`select()`][].
...
class UnhandledSelectedError(SelectError, Generic[ReceiverMessageT_co]):
    ...
    This exception is raised when a [`select()`][] iteration
    finishes without a call to [`selected_from()`][] for
...
async def select(  # noqa: DOC503
    *receivers: Receiver[Any],
    ...
    Note:
        ...
        * Use [`merge()`][]: this is useful when you have an
