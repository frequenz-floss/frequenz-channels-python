...
[`ChannelError`][..ChannelError] exception. When there is an attempt to
use a closed channel, a [`ChannelClosedError`][..ChannelClosedError]
exception is raised.

# Causes

When an exception is caused by another exception, for example if the underlying channel
was closed while sending or receiving a message, the original exception will be
...
try:
    ...
    match error.__cause__:
        case None:
            print("The message couldn't be sent for a known reason")
...
Tip:
    ...
    [`ReceiverStoppedError`][..ReceiverStoppedError] exception by
    explicitly calling [`receive()`][..Receiver.receive] on the
