...
[`Sender`][..Sender] objects. [`Sender`][..Sender]s are
usually created by calling `channel.new_sender()`, and are a very simple abstraction
that only provides a single [`send()`][..Sender.send] method:
...
Although [`send()`][..Sender.send] is an asynchronous method, some
...
[`send()`][..Sender.send] never blocks. However, please keep in mind
that the [asyncio][] event loop could give control to another task at any time,
effectively making the [`send()`][..Sender.send] method blocking.
...
a [`SenderError`][..SenderError] exception is raised.
...
class SenderError(Error, Generic[SenderMessageT_co]):
    """An error that originated in a [`Sender`][..Sender].
    ...
    def __init__(self, message: str, sender: Sender[SenderMessageT_co]):
        ...
        Args:
            ...
            sender: The [`Sender`][...Sender] where the error
                happened.
        """
        super().__init__(message)
        self.sender: Sender[SenderMessageT_co] = sender
        """The [`Sender`][...Sender] where the error happened."""
...
class SubscribableSender(Sender[SenderMessageT_contra], ABC):
    ...
    def subscribe(self) -> Receiver[SenderMessageT_contra]:
        ...
        Returns:
            A new [`Receiver`][...Receiver] attached to this sender's channel.
        """


class CloneableSender(Sender[SenderMessageT_contra], ABC):
    """A [`Sender`][..Sender] that can be cloned."""
    ...
    def clone(self) -> CloneableSender[SenderMessageT_contra]:
        ...
        Returns:
            A new [`CloneableSender`][...CloneableSender]
...
class CloneableSubscribableSender(
    ...
    """A [`Sender`][..Sender] that can be both cloned and subscribed to."""
