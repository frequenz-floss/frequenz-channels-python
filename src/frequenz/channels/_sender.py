...
Although [`send()`][] is an asynchronous method, some
...
[`send()`][] never blocks. However, please keep in mind
that the [asyncio][] event loop could give control to another task at any time,
effectively making the [`send()`][] method blocking.
...
a [`SenderError`][] exception is raised.
...
class SenderError(Error, Generic[SenderMessageT_co]):
    ...
    def __init__(self, message: str, sender: Sender[SenderMessageT_co]):
        ...
        Args:
            ...
            sender: The [`Sender`][] where the error
                happened.
        """
...
class CloneableSender(Sender[SenderMessageT_contra], ABC):
    ...
    def clone(self) -> CloneableSender[SenderMessageT_contra]:
        ...
        Returns:
            A new [`CloneableSender`][] that sends messages to the same channel as this sender.
        """
...
class CloneableSubscribableSender(
    ...
    """A [`Sender`][] that can be both cloned and subscribed to."""
